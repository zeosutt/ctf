...
void debugptr (const char *ptr) {
#ifdef ENABLE_VO_DEBUG_MESSAGES
  printf ("\t\tpc: %d\t[ptr-1, ptr, ptr+1] => [0x%x 0x%x 0x%x]\n", ptr - program_base, *(ptr-1), *ptr, *(ptr+1));
#endif
}
...
void assign_variable_command (const char *&ptr) {
  ...
  const char type = *ptr;
  debug("type (1: char, 2: int, 3: pointer): ", type);
  ++ptr;
...
int64 get_argument (const char *&ptr) {
  ...
  if (type == 0x01) {
    ...
    last_accessed_pointer = 0;
    debug("\b\b\b\b    --- comparison result: ", res);
    return res;
  } else if (type == 0x07) {
    ...
    last_accessed_pointer = 0;
    debug("\b\b\b\b    \t", left - right);
    return left - right;
  } else if (type == 0x0F) {
    debug ("get_argument: 0x0F -> printf call");
    last_accessed_pointer = 0;
    auto res = printf_command (ptr);
    debug("\b\b\b\b    \t", res);
    return res;
  } else if (type == 0x10) {
    debug ("get_argument: 0x10 -> strcmp call");
    last_accessed_pointer = 0;
    auto res = strcmp_command (ptr);
    debug("\b\b\b\b    \t", res);
    return res;
  } else if (type == 0x11) {
